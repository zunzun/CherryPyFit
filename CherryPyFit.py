...
class WebFitter(object):
    
    @cherrypy.expose
    def index(self):
        # return HTML to CherryPy as a web page
        s = '<html><body>'
        s += '<table><tr>'
    ...
    @cherrypy.expose
    def simplefitter_2D(self, textdata, equation, target):
        formTextData = textdata
        formEquation = equation
        formFittingTarget = target
    ...
    @cherrypy.expose
    def simplefitter_3D(self, textdata, equation, target):
        formTextData = textdata
        formEquation = equation
        formFittingTarget = target
...
if __name__ == '__main__':
    ...
    cherrypy.quickstart(WebFitter(), '/', conf)
